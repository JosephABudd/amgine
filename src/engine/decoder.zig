...
pub const Decoder = struct {
    ...
    pub fn decode(self: *Decoder, input: []const u8) ![]u8 {
        // 1. Reset the secret.
        self.secret.reset();
    ...
    /// decode_deep decode an []u8 returned by Encoder.encode().
    /// Param allocator is the alloctator used to create temp data and the returned []u8;
    /// Param input is the []u8 returned by Encoder.encode().
    /// Returns the decoded bytes or the error.
    pub fn decode_deep(self: *Decoder, input: []const u8) ![]u8 {
        // 1. Reset the secret.
        self.secret.reset();
        ...
        while (input_i < input.len) : ({
            ...
            // Get this secret's current rotor.
            var rotors = try self.secret.currentRotorsReverse(self.allocator);
            const last: usize = rotors.len - 1;
            if (rotors[last].isNoisey()) {
                // Ignore the noise.
                input_i += 1;
                output_i += 1;
            }
            // Decode each input through every rotor.
            var value: u8 = input[input_i];
            for (rotors) |rotor| {
                value = rotor.decode(value);
            }
...
pub fn init(allocator: std.mem.Allocator, secret: *_secret_.Secret) !*Decoder {
    ...
    decoder.secret = try secret.copy();
